...
class HingeLoss(nn.Module):
    ...
    def forward_D(self, fake_data: Tensor, real_data: Tensor):
        ...
        loss = torch.mean(F.relu(1 - real_score) + F.relu(1 + fake_score)) / 2
        if self.lambda_r1_reg > 0.0:
            loss = loss + self.lambda_r1_reg / 2 * self.r1_reg(real_data)
        return loss

    def forward(self, mode: str, fake_data: Tensor, real_data: Tensor = None):
        if mode == 'G':
            return self.forward_G(fake_data)
        elif mode == 'D':
            return self.forward_D(fake_data, real_data)
        else:
