...
from losses.vqgan import VQGANLoss
...
def main():
    ...
    valid_set = Subset(valid_set, torch.randperm(len(valid_set))[:32])
    ...
    # DEFINE LOSSES
    vqgan_loss = VQGANLoss(
        disc=disc, type_rec=conf.train.type_rec, coef_rec=conf.train.coef_rec, coef_lpips=conf.train.coef_lpips,
        coef_commit=conf.train.coef_commit, coef_vq=conf.train.coef_vq, start_adv=conf.train.start_adv,
        coef_adv=conf.train.coef_adv, adaptive_adv_weight=conf.train.adaptive_adv_weight,
    ).to(device)
    ...
    def run_step(batch):
        ...
        for i in range(0, bs, micro_bs):
            ...
            with model_no_sync, disc_no_sync:
                out = model(micro_x)
                losses = vqgan_loss(
                    step=step, recx=out['decx'], x=micro_x, z_e=out['z'], z_q=out['quantized_z'],
                    last_layer=unwrapped_model.decoder.conv_out[-1].weight,  # TODO: only suits for vqgan_net.Decoder
                )
        ...
        for i in range(0, bs, micro_bs):
            ...
            with model_no_sync, disc_no_sync:
                if step < conf.train.start_adv:
                    ...
                    out = model(micro_x)
                    loss_adv_d = vqgan_loss.hinge_loss('D', out['decx'], micro_x)
    ...
    @torch.no_grad()
    def sample(savepath):
        x = torch.stack([discard_label(i) for i in valid_set], dim=0).float().to(device)
