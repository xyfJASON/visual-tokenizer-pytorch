...
class SSIM:
    ...
    def __call__(self, img1: Tensor, img2: Tensor):
        ...
        ssim_list = [ssim(im1, im2, data_range=1.0, channel_axis=-1) for im1, im2 in zip(img1, img2)]
        ssim_list = torch.tensor(ssim_list, device=device)
        return reduce_tensor(ssim_list, self.reduction)
