...
class CosineMinimumWarmupLR(LambdaLR):
    """Cosine learning rate scheduler with warmup and minimum learning rate."""
    def __init__(self, optimizer: Optimizer, warmup_steps: int, training_steps: int, min_lr: float = 0.0, num_cycles: float = 0.5, last_epoch: int = -1):
        self.warmup_steps = warmup_steps
        self.training_steps = training_steps
        self.min_lr = min_lr
        self.num_cycles = num_cycles
    ...
    def lr_lambda(self, current_step):
        if current_step < self.warmup_steps:
            return float(current_step) / float(max(1, self.warmup_steps))
        progress = float(current_step - self.warmup_steps) / float(max(1, self.training_steps - self.warmup_steps))
        cos_term = 0.5 * (1.0 + math.cos(math.pi * float(self.num_cycles) * 2.0 * progress))
        return max(0.0, cos_term + self.min_lr - self.min_lr * cos_term)
